...
class CellsManagerClassTestCase(test.NoDBTestCase):
    ...
    def _test_aggregate_method(self, method_name, *args, **kwargs):
        expected_aggregate = objects.Aggregate(name="fun2", id=42)
        expected_response = cells_utils.AggregateProxy(expected_aggregate,
                                                       'fake')
        self.mox.StubOutWithMock(
            self.cells_manager.msg_runner, method_name)
        to_be_called = getattr(self.cells_manager.msg_runner, method_name)
        response = self._get_fake_response(expected_aggregate)
        to_be_called(*args, **kwargs).AndReturn(response)
        self.mox.ReplayAll()
        method = getattr(self.cells_manager, method_name)
        response = method(*args, **kwargs)
        self.assertEqual(expected_response._cell_path, response._cell_path)
        self.assertEqual(expected_response._obj, response._obj)
    ...
    def test_get_aggregate_list(self):
        expected_aggregates = [objects.Aggregate(name="fun2", id=42)]
        expected_response = [cells_utils.AggregateProxy(aggregate, 'fake') for
                             aggregate in expected_aggregates]
        response = self._get_fake_response(expected_aggregates)
        self.mox.StubOutWithMock(
        ...
        self.assertEqual(expected_response[0]._cell_path, response[0]._cell_path)
        self.assertEqual(expected_response[0]._obj, response[0]._obj)
